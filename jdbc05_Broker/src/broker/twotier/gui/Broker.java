	...
	*/
	...
	//==================================================================
		...
		*argument로 받은 CustomerRec[]을 stockList에 하나씩 뿌려준다.<BR>
		 ...
		 */    
		public void showList(ArrayList<SharesRec> portfolio, List list){
			list.removeAll();
		...
		public void showCustomer(){
			...
			try{
				...
				ArrayList<SharesRec> list=cr.getPortfolio();
				if(list !=null){ //주식을 보유한 고객이라면...
					showList(list, portList);
	...
	public static void main(String args[])throws Exception { // main은 기본쓰레드이다 
	...
	// 쓰레드가 작업하는 코드를 run() 안에 구현..
	// 3초마다 DB가서 상장된 주식의 정보를 가져오는 일만 전담하는 쓰레드를 작성
	@Override
	public void run() {
		while(true) {// 무한루핑을 돌면서... 3초마다 한번씩 DB의 모든 주식을 가져온다.
			try {
				showStockList(db.getAllStocks(), stockList);
				System.out.println("$$$$$$$$$$$$$$$$$$$$$$ 실시간 주식정보 받아옴 $$$$$$$$$$$$$$$$$");
				Thread.sleep(3000);
			} catch (Exception e) {
			}
		} 
